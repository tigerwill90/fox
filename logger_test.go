...
func TestLoggerWithHandler(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			want: "time=time level=INFO msg=ip status=200 method=GET path=/success latency=latency\n",
		},
		{
			name: "should log error level",
			req:  httptest.NewRequest(http.MethodGet, "/failure", nil),
			want: "time=time level=ERROR msg=ip status=500 method=GET path=/failure latency=latency\n",
		},
		{
			name: "should log warn level",
			req:  httptest.NewRequest(http.MethodGet, "/foobar", nil),
			want: "time=time level=WARN msg=ip status=404 method=GET path=/foobar latency=latency\n",
		},
		{
			name: "should log debug level",
			req:  httptest.NewRequest(http.MethodGet, "/success/", nil),
			want: "time=time level=DEBUG msg=ip status=301 method=GET path=/success/ latency=latency location=../success\n",
		},
	}
